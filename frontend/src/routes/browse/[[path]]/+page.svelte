<script>
  ...
  async function openTagView(tag) {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        const base = pathToUrlSegments($currentPath) ? `/browse/${pathToUrlSegments($currentPath)}` : '/browse';
        goto(`${base}?tag=${encodeURIComponent(tag)}`);
      }
    } catch (e) {
      tagViewFiles = [];
    } finally {
  ...
  onMount(async () => {
    const tagFromUrl = typeof window !== 'undefined' && $page.url.searchParams.get('tag');
    if (tagFromUrl) {
  ...
  function navigateToFolder(path) {
    ...
    if (browsingZip && path === '') {
      ...
      browsingZip = null;
      zipContents = [];
      goto(pathToUrlSegments(parentPath) ? `/browse/${pathToUrlSegments(parentPath)}` : '/browse');
    } else {
      browsingZip = null;
      zipContents = [];
      goto(path ? `/browse/${pathToUrlSegments(path)}` : '/browse');
    }
  }
...
<div class="app">
  ...
  <div class="breadcrumb">
    ...
    {:else}
      <button class="breadcrumb-item" onclick={() => goto('/browse')}>Root</button>
      {#each $currentPath.split('/').filter(Boolean) as part, i}
        <span class="breadcrumb-sep">/</span>
        <button 
          class="breadcrumb-item" 
          onclick={() => goto(pathToUrlSegments($currentPath.split('/').slice(0, i + 1).join('/')) ? '/browse/' + pathToUrlSegments($currentPath.split('/').slice(0, i + 1).join('/')) : '/browse')}
        >
          {part}
        </button>
      {/each}
